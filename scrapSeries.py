...
from db import insert_series, confirmation, connect_to_db
...
def scrap_series(list_url, conn):
    ...
    if response.status_code == STATUS_OK:
        ...
        for title in titles:
            ...
            insert_series(title_text, category_text, conn)  # Passando conn para a função            
    else:
        print(f"Falha ao acessar {list_url}. Erro {response.status_code}")


# Esta função abre os links em "leia mais". A cada link aberto, é executado scrap_series.
def acess_series_lists(base_url, conn):
    ...
    if response.status_code == STATUS_OK:
        ...
        html = response.text
        soup = BeautifulSoup(html, "html.parser")
        read_more = soup.find_all("div", {"class" : "read-more"})

        for div in read_more:
            link = div.find("a")
            if link:
                list_url = link.get("href")
                print(f'Link: {list_url}')
                scrap_series(list_url, conn)
    else:
...
# Executar o programa
conn = connect_to_db()
acess_series_lists(base_url, conn)
confirmation(conn)